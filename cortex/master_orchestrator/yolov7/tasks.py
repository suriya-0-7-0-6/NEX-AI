...
def detect(self, params):
    set_active_arch(params['modelinfo']['dnnarch'])
    map_location = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    global _yolov7_model
    if _yolov7_model is None:
        _yolov7_model = load_model(weights=params['modelinfo']['weights_path'], map_location=map_location)
    return detect_using_yolov7(_yolov7_model, map_location, params)
